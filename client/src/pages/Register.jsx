...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (form.password.length < 6) return toast.error('Fjalëkalimi duhet të jetë të paktën 6 karaktere');
    setLoading(true);
    try {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-beige-50">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Krijo llogarinë</h1>
          <p className="text-gray-500 mt-1 text-sm">Nis garderoben tënde dixhitale sot</p>
        </div>
        ...
        <div className="card p-8">
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            Ke llogari?{' '}
            <Link to="/login" className="font-medium text-gray-900 hover:underline">Hyr</Link>
          </p>
        </div>
      </div>
    </div>
